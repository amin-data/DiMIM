...
class MultiCropWrapper(nn.Module):
    ...
    def forward(self, x, bool_masked_position = None):
        ...
        cls_token = self.backbone(concatenated, bool_masked_position = bool_masked_position) #(n_samples * n_crops, out_dim)
...
class MaskingGenerator():
    ...
    def __init__(self, input_size, num_masking_patches = 20, min_num_patches = 3, max_num_patches = 20, 
                 min_aspect = 0.3, max_aspect = None, patch_size = 4):
        '''
        Parameters
        ----------
        input_size: (int, int)
                    It should be (num_patches, num_patches), meaning (image_size / patch_size, image_size / patch_size)
        '''      
        ...
        self.n_patches = self.width * self.height

        max_aspect = max_aspect or 1/min_aspect
        self.log_aspect_ratio = (math.log(min_aspect), math.log(max_aspect))


    def mask_(self, mask, max_mask_patches):
    ...
    def __call__(self):
        mask = np.zeros(shape=(self.width, self.height), dtype=np.int)
        mask_count = 0
        while mask_count < self.n_patches:
            max_mask_patches = self.num_masking_patches - mask_count
            max_mask_patches = min(max_mask_patches, self.num_masking_patches)

            delta = self.mask_(mask, max_mask_patches)
            if delta == 0:
                break
            else:
                mask_count += delta   
